...
\begin{figure}[h!]
  \centering
    \includegraphics[width=0.85\textwidth]{ThreadsPassing} 
	\caption{Threads passing tests}
\end{figure}

\begin{figure}[h!]
  \centering
    \includegraphics[width=0.85\textwidth]{01} 
	\caption{Starting of tests}
\end{figure}

\begin{figure}[h!]
  \centering
    \includegraphics[width=0.85\textwidth]{reportsWorking} 
	\caption{Reports passing the tests}
\end{figure}

\begin{figure}[h!]
  \centering
    \includegraphics[width=0.85\textwidth]{reportsWorking2} 
	\caption{Reports passing the tests}
\end{figure}

\begin{figure}[h!]
  \centering
    \includegraphics[width=0.85\textwidth]{ServerCrashOnLogInAttempt} 
	\caption{Server crashing on login attempt}
\end{figure}

\begin{figure}[h!]
  \centering
    \includegraphics[width=0.85\textwidth]{serverFeedBack} 
	\caption{Server feedback}
\end{figure}

\begin{figure}[h!]
  \centering
    \includegraphics[width=0.85\textwidth]{testConstraints} 
	\caption{Testing constraints}
\end{figure}

\begin{figure}[h!]
  \centering
    \includegraphics[width=0.85\textwidth]{testPost} 
...
The system satisfies scalability in the sense that it is currently able to handle all the modules for the computer science department. This is however only a small portion of what the system should be capable of doing. The system should theoretically be able to scale in order to service 50 000 students. This should not be a problem for the system, due to the technologies that it is using. Node js is a lot more scalable than Apache, even though it is only single threaded by default. The use of MongoDB, rather than MySql also increases the scalability of the server [1].  The scalability of Node js can be increased by using multi-core CPUs rather than a single core CPU and load balancers can be implemented. This will ensure that the server application scales more than enough in order to be able to serve the required amount of users. This can however only be done when purchasing more efficient hardware for the host machine. 
...
The performance for the reporting use cases exceeded expectations by performing well under the recommended 5s. This might however change as the database size increases. Testing was limited to the usage of a fairly small sized database. 
...
\begin{figure}[h!]
  \centering
    \includegraphics[width=0.85\textwidth]{addConstraint} 
\end{figure}

\begin{figure}[h!]
  \centering
    \includegraphics[width=0.85\textwidth]{addConstraint1} 
\end{figure}
...
The system is usable because firstly the necessary actions that a user can take appear in a navigation bar at the top of the screen. Thus it is easy for a new user to use and navigate the website. 
...
The interface is not cluttered, and only basic functionality is displayed on the home screen, making the system easy to understand. The buttons are labelled with text rather than with graphical icons, and the text on the button is quite explanatory, which makes their purpose more clear. 
...
Larger headings are used to label the different sections, for example, under the Manage Constraints tab there are large headings to indicate the existing constraints section and the add new constraint section. This again contributes to ease of use for new users.

Through these mechanisms the system is easy to understand and use.  

\item Integratibility:

The system is able to address future integration requirements by providing access to its services using widely adopted public standards such as having separate npm packages for all the different modules. The packages are stored on Synopia. Electrolyte is also being used in the server to provide a dependency injection. The HandleBars server is the main server that needs to be used to test and integrate all the modules on. The routes/index.js, routes/infrastructure.js, and routes/content.js files use express to route the different hbs files for the different modules in order to integrate the infrastructure and content subsystems into the main system. 
...
A separate file is used to establish the connection to the database to avoid having this done in all the separate files. Global variables are also used, for example, the global password and username. 
...
This package can easily be swapped out and an alternative data source package with different credentials and roles can be used due to the pluggability of the system and dependency injection employed by the top level team. The new package must be named the same as the old one to avoid errors where the file is required from the package in the code. 
...
\input{IntegratibilityB}
\input{DeployabilityB}
\end{enumerate}
